...
var docRef;
var SR_poseTimeSpent = [];
var SL_poseTimeSpent = [];
var S_poseTimeSpent = [];
...
async function getExercise(firestoreDocPath) {
  docRef = doc(firestore, firestoreDocPath);
  ...
  if (dataObj["isBegin"] === true && dataObj["isTestExercise"] != true) {
    ...
    S_poseTimeSpent = dataObj["analysis"]["poseTimeSpent"];
    prevFlag = dataObj["analysis"]["prevFlag"];
    Counter = dataObj["analysis"]["counter"];
    SR_sets = dataObj["analysis"]["SR_sets"];
    SL_sets = dataObj["analysis"]["SL_sets"];
    S_Sets = dataObj["analysis"]["S_Sets"];
  }
  else
  {
    firstTime = true;
    for (let i = 0; i < timings.length; i++) {
      SR_poseTimeSpent.push(0);
      SL_poseTimeSpent.push(0);
      S_poseTimeSpent.push(0);
    }

    console.log("initializing analysisData");
    const analysisData = {
      analysis: {
        counter: 0,
        prevFlag: 0,
  ...
  //! initialize from firebase , initially = zero

  if (firstTime) { SR_sets = sets; SL_sets = sets; S_Sets = sets; }
  else {
    if (flag != "RTL") { sets = S_Sets; }
    else {
      if (flag == "RTL" && prevFlag == 0) { sets = SR_sets; }
      else  {
        sets = SL_sets;
        flag = "L";
      }
  ...
  if(S_Sets == 0 || (SR_sets == 0 && SL_sets == 0)){
    endPoseDetection();
  ...
  for (let index = 0; index < timings.length; index++) {
    if (timings[index] < 1) { poseTiming[index + 1] = 0 }
    else { poseTiming[index + 1] = timings[index]; }

  }
  console.log('poseTiming : ',poseTiming);
  window.downloadFile(exercisePath).then((txt) => {
    ...
    for (let i = 1; i <= pose_angles_list.length; i++) {
      poseTimeSpent[i] = S_poseTimeSpent[i-1];
      R_poseTimeSpent[i] = SR_poseTimeSpent[i-1];
      L_poseTimeSpent[i] = SL_poseTimeSpent[i-1];
      poseAcc[i] = 0;
      R_poseAcc[i] = 0;
      L_poseAcc[i] = 0;
    }
    console.log("fixed : ", fixedAngles)
    console.log("dynamic : ", dynamicAngles)
...
function comparePoses(RT_Angles, Correct_Angles) {
  ...
  for (let i = 0; i < RT_Angles.length; i++) {
    ...
    // if (rt_angle > 0 && correct_angle < 0)
    // {
    //   correct_angle += 360;  
    // }
    // else if (rt_angle < 0 && correct_angle > 0)
    // {
    //   rt_angle += 360;
    // }
    if (customizedAngles[i][2] == "N") {
      ...
      if (checkTolerance(rt_angle,correct_angle,tolerance_deg)) {
        Angle_Match += 1;
      }
    }
  }
...
function checkTolerance(a, b, x) {  
  // Calculate the absolute difference
  let diff = Math.abs(a - b);
  
  // Consider the wrap-around
  let wrappedDiff = 360 - diff;
  
  // Check if either difference is within x degrees
  return diff <= x || wrappedDiff <= x;
}
...
function onResultsPose(results) {
  ...
  if (!Match && inPose) {
    ...
    totalDuration += poseDuration;
    if (poseTiming[Num_Of_Pose_Completed + 1] > 0) {
      if (flag == "RTL") {
        R_poseTimeSpent[Num_Of_Pose_Completed + 1] += poseDuration; // Add duration to total pose time
        if (totalDuration > poseTiming[Num_Of_Pose_Completed + 1]) { R_poseTimeSpent[Num_Of_Pose_Completed + 1] -= (totalDuration - poseTiming[Num_Of_Pose_Completed + 1]) }
      }
      else if (prevFlag == 1) {
        L_poseTimeSpent[Num_Of_Pose_Completed + 1] += poseDuration; // Add duration to total pose time
        if (totalDuration > poseTiming[Num_Of_Pose_Completed + 1]) { L_poseTimeSpent[Num_Of_Pose_Completed + 1] -= (totalDuration - poseTiming[Num_Of_Pose_Completed + 1]) }
      }
      else {
        poseTimeSpent[Num_Of_Pose_Completed + 1] += poseDuration; // Add duration to total pose time
        if (totalDuration > poseTiming[Num_Of_Pose_Completed + 1]) { poseTimeSpent[Num_Of_Pose_Completed + 1] -= (totalDuration - poseTiming[Num_Of_Pose_Completed + 1]) }
      }
    }
  ...
  poseInfoElement.textContent = `Make pose number : (${Num_Of_Pose_Completed + 1
    })`;
  ...
  let timer = setInterval(() => {
    ...
    if (countdown <= 0) {
      ...
      if (Num_Of_Pose_Completed + 1 == pose_angles_list.length) {
        console.log()
        console.log("Time Spent at pose : ", [Num_Of_Pose_Completed + 1], " on right = ", R_poseTimeSpent[Num_Of_Pose_Completed + 1])
        console.log("Time Spent at pose : ", [Num_Of_Pose_Completed + 1], " on left = ", L_poseTimeSpent[Num_Of_Pose_Completed + 1])
        console.log("Time Spent at pose : ", [Num_Of_Pose_Completed + 1], " = ", poseTimeSpent[Num_Of_Pose_Completed + 1])
        Counter += 1;

        //! Send Counter to firebase >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        if (params["dataPath"]) {
          updateDoc(docRef, { "analysis.counter": Counter });
        }
      ...
      if (Counter >= counterMax) {
        ...
        if (flag == "RTL") {
          SR_sets = sets;
          //& Note :  number of sets completed =  Tsets(defined by doctor) - Jsets(From the js code)
          //& Note : They will be updated at the end of each set 
          //! Send SR_sets  to firebase 
          if (params["dataPath"]) {
            updateDoc(docRef, { "analysis.SR_Sets": SR_sets });
          }
        }
        else if (prevFlag == 1) {
          SL_sets = sets;

          //! Send SL_sets to firebase 
          if (params["dataPath"]) {
            updateDoc(docRef, { "analysis.SL_Sets": SL_sets });
          }
        }
        else {
          //! Send sets to firebase 
          S_Sets = sets ; 
          if (params["dataPath"]) {
            updateDoc(docRef, { "analysis.S_Sets": S_Sets });
          }
        }

        if
          (sets > 0) {
          breakTxt = "Good job, Take a break!"
          Counter = 0;
          stopPoseDetection();
          displayBreakTime();
        }
        else if (flag == "RTL") {
          breakTxt = "Good job, switch to left side!"
          flag = "L";
          prevFlag = 1;
          //! send prevFlag to firebase >>>>>>>>>>>>>>>>>>>>>>>>>>
          updateDoc(docRef, { "analysis.prevFlag": prevFlag });
          Counter = 0;
          //! initialize from firebase (Dsets)>>>>>>>>>>>>>>>>>>>>>>>>>>> (i.e D_sets)
          sets = SL_sets;
          customizedAngles = [];
          customizeAngles();
          stopPoseDetection();
      ...
      //& Note : Rght side TimeSpent vars will be saved for each day separately
      //& Note : initially they will be set to zero and updated using (+= not overwrite) 
      //& Note : They will be updated at the end of each rep 
      //! Send TimeSpent at pose number (Num_Of_Pose_Completed + 1) using right side to firebase >>>>>>>> 
      //! Send TimeSpent at pose number (Num_Of_Pose_Completed + 1) using left side to firebase 
      //! Send TimeSpent at pose number (Num_Of_Pose_Completed + 1) to firebase 

      if (params["dataPath"]) {
        updateDoc(docRef, {
          "analysis.R_poseTimeSpent": R_poseTimeSpent,
          "analysis.L_poseTimeSpent": L_poseTimeSpent,
          "analysis.poseTimeSpent": poseTimeSpent,
        });
      }
      totalDuration = 0;
...
function displayBreakTime() {
  ...
  const x = canvasWidth / 2 - textWidth / 2;
  const y = canvasHeight / 2;
  canvasCtx.fillText(breakTxt, x, y);
